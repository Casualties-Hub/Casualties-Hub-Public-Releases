...
public class ModService
{
    ...
    public string GetPluginsPath(Settings settings)
    {
        ...
        if (Path.GetFileName(normalized).Equals("BepInEx", StringComparison.OrdinalIgnoreCase))
            return LinuxPaths.ResolveChild(normalized, "Plugins");
        return LinuxPaths.ResolveChain(normalized, "BepInEx", "Plugins");
    }
    ...
    public int SetAllModsDisabled(Settings settings, bool disabled)
    {
        ...
        var files = disabled
            ? Directory.EnumerateFiles(pluginsPath, "*.dll", LinuxPaths.CaseInsensitiveRecursive).ToList()
            : Directory.EnumerateFiles(pluginsPath, "*.dll.disabled", LinuxPaths.CaseInsensitiveRecursive).ToList();
    ...
    private static void InstallCustomSprite(string pluginsPath, string stagingPath, string? skinSlot)
    {
        ...
        var sourceFile = Directory.EnumerateFiles(stagingPath, "experimentCrus.png", LinuxPaths.CaseInsensitiveRecursive)
            .FirstOrDefault();
        ...
        var destination = LinuxPaths.ResolveChild(
            LinuxPaths.ResolveChild(pluginsPath, "CustomSprites"), skinSlot.ToLowerInvariant());
    ...
    public static string BackupRoot(Settings settings) =>
        Path.IsPathRooted(settings.BackupPath)
            ? settings.BackupPath
            : Path.Combine(LinuxPaths.AppDataRoot(), settings.BackupPath);
