...
public sealed class HubPathsTests : IDisposable
{
    ...
    public void FindChild_matches_regardless_of_case()
    {
        ...
        var found = HubPaths.FindChild(_root, "Plugins");
    ...
    public void FindChild_prefers_the_exact_match_when_both_exist()
    {
        ...
        Assert.Equal("Plugins", Path.GetFileName(HubPaths.FindChild(_root, "Plugins")));
    }

    [Fact]
    public void FindChild_returns_null_when_absent()
    {
        Assert.Null(HubPaths.FindChild(_root, "Plugins"));
        Assert.Null(HubPaths.FindChild(Path.Combine(_root, "nope"), "Plugins"));
    }

    [Fact]
    public void ResolveChild_falls_back_to_exact_case_for_creation()
    {
        var resolved = HubPaths.ResolveChild(_root, "BepInEx");
    ...
    public void ResolveChain_walks_every_level_case_insensitively()
    {
        ...
        var resolved = HubPaths.ResolveChain(_root, "BepInEx", "Plugins");
    ...
    public void IsInside_only_accepts_real_descendants(string candidate, string container, bool expected)
    {
        ...
        Assert.Equal(expected, HubPaths.IsInside(candidate, container));
    }
    ...
    {
        Assert.False(HubPaths.IsInside("/games/plugins/../../etc/passwd", "/games/plugins"));
    }
