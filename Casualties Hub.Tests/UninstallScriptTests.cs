...
public sealed class UninstallScriptTests
{
    ...
    {
        var escaped = Path.Combine(HubPaths.AppDataRoot(), "..", "..", "..", "etc");
    ...
    public void The_hub_data_directory_is_allowed()
    {
        ...
        Assert.True(UninstallService.IsSafeToDelete(Path.Combine(HubPaths.AppDataRoot(), "Logs")));
    }
    ...
    public void Unsafe_paths_are_dropped_before_the_script_is_built()
    {
        ...
        {
            new() { Key = "Bad", Title = "Bad", Description = "test", Paths = ["/", "/etc", Path.Combine(HubPaths.AppDataRoot(), "Logs")] },
        };
    ...
    {
        var logs = Path.Combine(HubPaths.AppDataRoot(), "Logs");
        var items = new List<UninstallItem>
        {
