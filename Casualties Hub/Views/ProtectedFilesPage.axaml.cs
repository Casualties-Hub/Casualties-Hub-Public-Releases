...
public partial class ProtectedFilesPage : UserControl
{
    ...
    private async Task ProtectAsync()
    {
        ...
        var outside = paths.Where(path => !HubPaths.IsInside(path, plugins)).ToList();
        if (outside.Count > 0)
        {
    ...
    private async Task ProtectFolderAsync()
    {
        ...
        if (paths.Any(path => HubPaths.IsInside(plugins, path)))
        {
    ...
    private void OnOpenFolder(object? sender, RoutedEventArgs e)
    {
        ...
        if (Directory.Exists(root)) DesktopShell.OpenFolder(root);
    }
}
